...
public class CustomTrattaRepositoryImpl implements CustomTrattaRepository {
	...
	public List<Tratta> findByExample(Tratta example) {
		...
		StringBuilder queryBuilder = new StringBuilder("select distinct t from Tratta t where t.id = t.id ");
		...
		if(example.getAirbus() != null) {
			whereClauses.add("t.airbus.id = :idExample");
			paramaterMap.put("idExample", example.getAirbus().getId());
		}
